...
CPIDropshipInfo::CPIDropshipInfo(CPIWinControl *Parent) : CPIWinControl(Parent)
{
  mDropshipUnit   = 0;
...
CPIDropshipInfo::~CPIDropshipInfo()
{
  GetGraphicInstance()->ReleaseSprite(mSpriteInfantry);
  GetGraphicInstance()->ReleaseSprite(mSpriteVehicle);
}
//---------------------------------------------------------------------------

void CPIDropshipInfo::SetDropshipUnit(CIGDropship* _unit)
{
  if (!_unit)
  {
    mDropshipUnit = 0;
...
void CPIDropshipInfo::Paint()
{
  ...
  int lCount = mDropshipUnit->GetCapacity();
  int lX = 0;
  int lY = 0;
  ...
  {
    CIGBaseObject* lUnit = mDropshipUnit->GetUnit(i);
    ...
    // it is possible that the dropship may contain other units (because it is
    // a container of CIGUnit*) but i don't bother for now to fix it
    // ...
...
void CPIDropshipInfo::OnLeftMouseDown(int X, int Y)
{
  ...
  for(int i = 0; i < lCount; i++)
  {
    ...
    if ((lX < lPointClick.x) && (lPointClick.x < lX + 32) && (lY < lPointClick.y) && (lPointClick.y < lY + 32))
    {
      ...
      mDropshipUnit->SetAction(lTmpActionData);
    }
