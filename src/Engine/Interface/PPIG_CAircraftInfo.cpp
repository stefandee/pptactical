...
CPIAircraftInfo::CPIAircraftInfo(CPIWinControl *Parent) : CPIWinControl(Parent)
{
  CPFont* lFont;
  ...
  mVehicle->LoadFromFile(getGamePath() + PATH_PORTRAITS_GFX + "rion.png");
  */
...
CPIAircraftInfo::~CPIAircraftInfo()
{
  mAircraftUnit = 0;
}
//---------------------------------------------------------------------------

void CPIAircraftInfo::SetAircraftUnit(CIGAircraft* _unit)
{
  ...
  if (_unit->ClassName() != CPString(IGCN_FIGHTERUNIT) &&
      _unit->ClassName() != CPString(IGCN_CHOPPERUNIT) &&
      _unit->ClassName() != CPString(IGCN_BOMBERUNIT))
  {
    mAircraftUnit = 0;
    SetVisible(false);
    return;
  ...
  TVehicleUnified lData = mAircraftUnit->GetAircraftUnifiedData();
  ...
  {
    case 4:
      lVehicleName = lVehicleName + " (fighter)";
      break;
    case 5:
      lVehicleName = lVehicleName + " (bomber)";
      break;
    case 6:
      lVehicleName = lVehicleName + " (chopper)";
      break;
  }
...
CPString CPIAircraftInfo::GetWeaponName(int _type)
{
  switch(_type)
  {
